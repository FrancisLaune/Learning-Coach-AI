...
def is_correct(question: dict[str, Any], answer: str) -> bool:
    if question["answer_type"] == "number":
        try:
            ...
            if not match:
                return False
            actual = float(match.group(0)); expected = float(question["expected_answer"])
...
def create_question(chapter, question, expected_answer, explanation,
                    answer_type="text", unit="", accepted_answers=None):
    return {"chapter":chapter,"question":question,"expected_answer":expected_answer,
            "explanation":explanation,"answer_type":answer_type,"unit":unit,
...
def _generate(module: Any, chapter: str, difficulty: str) -> dict[str, Any]:
    try:
        ...
        q=module.generate_question(chapter, difficulty=difficulty) if "difficulty" in params else module.generate_question(chapter)
    except (TypeError, ValueError):
        q=module.generate_question(chapter)
    q=dict(q)
    q["difficulty"]=difficulty
    q.setdefault("target_seconds", {"Facile":45,"Moyen":70,"Difficile":100,"Brevet":130,"Expert":170}.get(difficulty,90))
    return q
...
def build_question_set(subject_module: Any, count: int, chapters=None,
                       difficulty: str="Moyen") -> list[dict[str,Any]]:
    selected=list(chapters or subject_module.CHAPTERS.keys())
    if not selected: raise ValueError("Au moins un chapitre doit être sélectionné.")
    result=[]; signatures=set(); attempts=0; max_attempts=max(250,count*40)
    cycle=[]
    while len(cycle)<count:
        block=selected.copy(); random.shuffle(block); cycle.extend(block)
