...
CHAPTERS = {
    "Nombres relatifs": {
        ...
        "key_points": [
            ...
            "Deux signes négatifs au produit donnent un résultat positif.",
        ],
    },
    "Fractions": {
        ...
        "key_points": [
            "Simplifier avec un diviseur commun.",
            "Dénominateur commun pour l'addition.",
            "Inverser la seconde fraction pour une division.",
        ],
    },
    "Puissances": {
    ...
    "Calcul littéral": {
        ...
        "key_points": [
            ...
            "Factoriser en faisant apparaître un facteur commun.",
        ],
    },
    "Équations": {
    ...
    "Pythagore": {
        ...
        "key_points": [
            ...
            "Réciproque pour prouver qu'un triangle est rectangle.",
        ],
    },
    "Thalès": {
        ...
        "key_points": [
            ...
            "Configuration avec droites parallèles.",
    ...
    "Fonctions": {
        ...
        "key_points": [
            ...
            "Résoudre f(x)=b cherche un antécédent.",
...
def generate_question(chapter: str, difficulty: str = "Moyen") -> dict:
    ...
    if difficulty in {"Moyen", "Difficile"} and random.random() < (0.45 if difficulty == "Moyen" else 0.70):
        if chapter in {"Pourcentages", "Proportionnalité"}:
            price = random.choice([120, 160, 240, 320])
            pct = random.choice([15, 20, 25, 30])
            qty = random.choice([2, 3, 4])
            final = price * (1 - pct / 100) * qty
            return create_question(
                chapter,
                f"Un magasin vend {qty} articles à {price} € chacun. Une remise de {pct} % est appliquée sur chaque article. Quel est le montant total payé ?",
                final,
                f"Prix remisé d’un article={price}×(1-{pct}/100)={price * (1 - pct / 100):g} €. Pour {qty} articles : {final:g} €.",
                "number",
                "€",
            )
        if chapter in {"Aires et volumes", "Conversions"}:
            ...
            height = random.choice([1.2, 1.5, 1.8])
            fill = random.choice([75, 80, 85, 90])
            litres = length * width * height * 1000 * fill / 100
            return create_question(
                chapter,
                f"Une piscine rectangulaire mesure {length} m sur {width} m et a une profondeur moyenne de {height:g} m. Elle est remplie à {fill} %. Quel volume d’eau contient-elle en litres ?",
                litres,
                f"Volume total={length}×{width}×{height:g}={length * width * height:g} m³, soit {length * width * height * 1000:g} L. À {fill} % : {litres:g} L.",
                "number",
                "L",
            )
        if chapter in {"Équations", "Calcul littéral"}:
            x = random.randint(4, 15)
            fixed = random.randint(3, 10)
            unit = random.randint(2, 8)
            total = fixed + unit * x
            return create_question(
                chapter,
                f"Une location coûte {fixed} € de frais fixes puis {unit} € par heure. La facture est de {total} €. Combien d’heures ont été facturées ?",
                x,
                f"On résout {fixed}+{unit}x={total}, donc x={x}.",
                "number",
                "h",
            )
        if chapter == "Pythagore":
            ...
            return create_question(
                chapter,
                f"Une échelle est posée contre un mur. Son pied se trouve à {a} m du mur et son sommet atteint {b} m de hauteur. Quelle est la longueur de l’échelle ?",
                c,
                f"L’échelle est l’hypoténuse : √({a}²+{b}²)={c} m.",
                "number",
                "m",
            )
    ...
    if chapter == "Fractions":
        ...
        answer = f"{n // g}/{den // g}"
        return create_question(
            chapter, f"Calcule et simplifie : {a}/{b}+{c}/{d}", answer, f"Après mise au même dénominateur : {answer}."
        )
    if chapter == "Puissances":
        ...
        answer = f"{base}^{m + n}"
        return create_question(
            chapter, f"Réduis : {base}^{m} × {base}^{n}", answer, f"On additionne les exposants : {answer}."
        )
    ...
    if chapter == "Conversions":
        ...
        return create_question(
            chapter, f"Convertis {km:g} km en mètres.", km * 1000, f"{km:g}×1000={km * 1000:g} m.", "number", "m"
        )
    if chapter == "Pythagore":
        a, b, c = random.choice([(3, 4, 5), (5, 12, 13), (6, 8, 10), (8, 15, 17)])
        return create_question(
            chapter,
            f"Triangle rectangle : côtés de l'angle droit {a} cm et {b} cm. Hypoténuse ?",
            c,
            f"c²={a * a}+{b * b}={c * c}, donc c={c}.",
            "number",
            "cm",
        )
    if chapter == "Thalès":
        ...
        return create_question(
            chapter,
            f"Une longueur de {a} cm correspond à {a * k} cm. À quoi correspondent {b} cm ?",
            b * k,
            f"Le coefficient est {k}, donc {b}×{k}={b * k}.",
            "number",
            "cm",
        )
    ...
    if chapter == "Probabilités":
        ...
        total = red + random.randint(2, 8)
        g = math.gcd(red, total)
        answer = f"{red // g}/{total // g}"
        return create_question(
            chapter,
            f"Un sac contient {red} boules rouges sur {total}. Probabilité d'obtenir rouge ?",
            answer,
            f"{red}/{total}={answer}.",
        )
    ...
    return create_question(
        chapter,
        "Une variable vaut 2. On répète 4 fois l'instruction « ajouter 3 ». Valeur finale ?",
        14,
        "2+4×3=14.",
        "number",
    )
