...
def test_load_openai_api_key_uses_environment_when_secrets_missing(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("OPENAI_API_KEY", "env-key")
    missing = tmp_path / "missing.toml"
    assert load_openai_api_key(secrets_path=missing) == "env-key"


def test_load_openai_api_key_prefers_secrets_over_environment(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
