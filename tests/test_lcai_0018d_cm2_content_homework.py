"""LCAI-0018D — CM2 homework availability and coverage tests."""
...
AUDIT_SCRIPT = ROOT / "scripts" / "lcai_0018d_phase0_audit.py"
GRADE = "FR-CM2"


def _cm2_has_published_content() -> bool:
...
def homework_database(tmp_path: Path) -> tuple[Path, int, int, int]:
    ...
    path = tmp_path / "lcai_0018d_cm2.duckdb"
    ...
    try:
        learner_id = int(
            connection.execute(
                "INSERT INTO learners(external_ref,display_name) VALUES ('student:cm2','Léa') RETURNING id"
            ).fetchone()[0]
        )
...
def test_cm2_curriculum_has_eight_subjects(homework_database: tuple[Path, int, int, int]) -> None:
...
def _cm2_published_count_for_subject(subject_code: str) -> int:
    connection = connect_v2(get_v2_database_path(), read_only=True)
    try:
        return int(
            connection.execute(
...
def test_mathematics_cm2_subject_has_active_chapters(homework_database: tuple[Path, int, int, int]) -> None:
...
def test_french_cm2_subject_has_active_chapters(homework_database: tuple[Path, int, int, int]) -> None:
    path, _learner_id, french_id, _math_id = homework_database
    repository = DuckDBUnifiedExperienceRepository(path)
    chapters = repository.chapters(french_id, _grade_id(path))
    if not chapters:
        pytest.skip("No active CM2 French chapters in current curriculum snapshot.")
    assert chapters


def test_homework_availability_lists_all_cm2_subjects(homework_database: tuple[Path, int, int, int]) -> None:
...
@pytest.mark.skipif(_cm2_published_count_for_subject("FRENCH") == 0, reason="No CM2 French content published")
def test_parent_can_generate_french_homework_for_cm2(homework_database: tuple[Path, int, int, int]) -> None:
...
@pytest.mark.skipif(_cm2_published_count_for_subject("MATHEMATICS") == 0, reason="No CM2 Mathematics content published")
def test_parent_can_generate_mathematics_homework_for_cm2(homework_database: tuple[Path, int, int, int]) -> None:
...
@pytest.mark.skipif(not _cm2_has_published_content(), reason="CM2 publication not yet executed")
def test_homework_can_be_generated_for_each_available_cm2_subject(
...
def coverage_exports() -> Path:
    path = EXPORTS / "LCAI-0018D_CM2_COVERAGE_MATRIX.csv"
    if not path.exists():
        subprocess.run([sys.executable, str(AUDIT_SCRIPT)], check=True, cwd=ROOT)
    assert path.exists(), "CM2 coverage matrix export missing"
    return path


def test_cm2_coverage_export_contains_all_configured_subjects(coverage_exports: Path) -> None:
...
def test_cm2_coverage_export_includes_chapter_rows(coverage_exports: Path) -> None:
    with coverage_exports.open(encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    assert len(rows) >= 100
