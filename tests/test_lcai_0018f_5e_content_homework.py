"""LCAI-0018F — 5e homework availability and coverage tests."""
...
AUDIT_SCRIPT = ROOT / "scripts" / "lcai_0018f_phase0_audit.py"
GRADE = "FR-5E"


def _5e_has_published_content() -> bool:
...
def homework_database(tmp_path: Path) -> tuple[Path, int, int, int]:
    ...
    path = tmp_path / "lcai_0018f_5e.duckdb"
    ...
    try:
        learner_id = int(
            connection.execute(
                "INSERT INTO learners(external_ref,display_name) VALUES ('student:5e','Léa') RETURNING id"
            ).fetchone()[0]
        )
...
def test_5e_curriculum_has_eight_subjects(homework_database: tuple[Path, int, int, int]) -> None:
...
def _5e_published_count_for_subject(subject_code: str) -> int:
    connection = connect_v2(get_v2_database_path(), read_only=True)
    try:
        return int(
            connection.execute(
...
def test_mathematics_5e_subject_has_active_chapters(homework_database: tuple[Path, int, int, int]) -> None:
...
def test_french_5e_subject_has_active_chapters(homework_database: tuple[Path, int, int, int]) -> None:
    path, _learner_id, french_id, _math_id = homework_database
    repository = DuckDBUnifiedExperienceRepository(path)
    chapters = repository.chapters(french_id, _grade_id(path))
    if not chapters:
        pytest.skip("No active 5e French chapters in current curriculum snapshot.")
    assert chapters


def test_homework_availability_lists_all_5e_subjects(homework_database: tuple[Path, int, int, int]) -> None:
...
def test_parent_can_generate_french_homework_for_5e(homework_database: tuple[Path, int, int, int]) -> None:
    if _5e_published_count_for_subject("FRENCH") == 0:
        pytest.skip("No 5e French content published")
...
def test_parent_can_generate_mathematics_homework_for_5e(homework_database: tuple[Path, int, int, int]) -> None:
    if _5e_published_count_for_subject("MATHEMATICS") == 0:
        pytest.skip("No 5e Mathematics content published")
...
def test_homework_can_be_generated_for_each_available_5e_subject(
    homework_database: tuple[Path, int, int, int],
) -> None:
    if not _5e_has_published_content():
        pytest.skip("5e publication not yet executed")
    path, learner_id, _french_id, _math_id = homework_database
    repository = DuckDBUnifiedExperienceRepository(path)
...
def coverage_exports() -> Path:
    path = EXPORTS / "LCAI-0018F_5E_COVERAGE_MATRIX.csv"
    if not path.exists():
        subprocess.run([sys.executable, str(AUDIT_SCRIPT)], check=True, cwd=ROOT)
    assert path.exists(), "5e coverage matrix export missing"
    return path


def test_5e_coverage_export_contains_all_configured_subjects(coverage_exports: Path) -> None:
...
def test_5e_coverage_export_includes_chapter_rows(coverage_exports: Path) -> None:
    with coverage_exports.open(encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    assert len(rows) >= 100
