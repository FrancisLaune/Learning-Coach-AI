...
def export_gap_by_chapter(matrix_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    by_chapter: dict[tuple[str, str], dict[str, object]] = {}
    for row in matrix_rows:
        ...
        bucket["skills"].add(row["skill_code"])
        bucket["approved_total"] += int(row["approved_count"])
        bucket["draft_total"] += int(row["draft_count"])
        if int(row["approved_count"]) == 0:
        ...
        gaps.append(
            {
                ...
                "publication_status": "published" if item["approved_total"] else ("draft_only" if item["draft_total"] else "empty"),
...
def write_baseline(
    *,
    ...
    zero_published_rows = sum(1 for row in matrix_rows if int(row["approved_count"]) == 0)
...
def main() -> None:
    ...
    if candidate_inventory:
        with (EXPORTS / "LCAI-0018C_CM1_CANDIDATE_INVENTORY.csv").open(
