"""LCAI-0018D Phase C0 — CM2 curriculum coverage and candidate baseline audit."""
...
CM2_RESOURCES = ROOT / "resources" / "content" / "cm2"
GRADE = "FR-CM2"
...
def load_candidate_packs() -> list[dict[str, object]]:
    ...
    for path in sorted(CM2_RESOURCES.glob("lcai_cm2_*_candidates_v1.json")):
...
def write_baseline(
    *,
    ...
    lines = [
        "# LCAI-0018D — Phase C0 : baseline CM2",
        "",
        f"- Niveau : **{GRADE}**",
    ...
    lines.extend(
        [
            ...
            "## Livrables C0",
            "",
            "- `docs/phase3/exports/LCAI-0018D_CM2_COVERAGE_MATRIX.csv`",
            "- `docs/phase3/exports/LCAI-0018D_CM2_GAP_BY_CHAPTER.csv`",
            "- `docs/phase3/exports/LCAI-0018D_CM2_CANDIDATE_INVENTORY.csv`",
            "- `docs/phase3/LCAI-0018D_GAP_ANALYSIS.md`",
            "",
            "## Prochaine étape (C1)",
            "",
            "1. Importer / valider les candidats CM2 via Content Factory et quality gates existants.",
            "2. Revue pédagogique humaine distincte de l'approbation.",
            "3. Publication production — recalcul coverage — validation moteurs.",
            "",
        ]
    )
    (DOCS / "LCAI-0018D_PHASE0_BASELINE.md").write_text("\n".join(lines), encoding="utf-8")
...
def write_gap_analysis(gaps: list[dict[str, object]], candidate_inventory: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# LCAI-0018D — Analyse des écarts CM2 (Gap Analysis C0)",
        "",
        f"- Chapitres curriculum CM2 : **{len(gaps)}**",
    ...
    lines.append("")
    (DOCS / "LCAI-0018D_GAP_ANALYSIS.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    if matrix_rows:
        with (EXPORTS / "LCAI-0018D_CM2_COVERAGE_MATRIX.csv").open("w", encoding="utf-8-sig", newline="") as handle:
            writer = csv.DictWriter(handle, fieldnames=list(matrix_rows[0].keys()))
            writer.writeheader()
            writer.writerows(matrix_rows)

    if gaps:
        with (EXPORTS / "LCAI-0018D_CM2_GAP_BY_CHAPTER.csv").open("w", encoding="utf-8-sig", newline="") as handle:
            writer = csv.DictWriter(handle, fieldnames=list(gaps[0].keys()))
            writer.writeheader()
            writer.writerows(gaps)

    if candidate_inventory:
        with (EXPORTS / "LCAI-0018D_CM2_CANDIDATE_INVENTORY.csv").open("w", encoding="utf-8-sig", newline="") as handle:
    ...
    print("LCAI-0018D Phase C0 audit complete")
