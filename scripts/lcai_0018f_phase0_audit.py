"""LCAI-0018F Phase C0 — 5e curriculum coverage and candidate baseline audit."""
...
FIVEE_RESOURCES = ROOT / "resources" / "content" / "5e"
GRADE = "FR-5E"
...
def load_candidate_packs() -> list[dict[str, object]]:
    ...
    for path in sorted(FIVEE_RESOURCES.glob("lcai_5e_*_candidates_v1.json")):
...
def write_baseline(
    *,
    ...
    lines = [
        "# LCAI-0018F — Phase C0 : baseline 5e",
        "",
        f"- Niveau : **{GRADE}**",
    ...
    lines.extend(
        [
            ...
            "",
            "Packs JSON dans `resources/content/5e/` — lifecycle Draft, approbation humaine obligatoire.",
            "",
            "## Livrables C0",
            "",
            "- `docs/phase3/exports/LCAI-0018F_5E_COVERAGE_MATRIX.csv`",
            "- `docs/phase3/exports/LCAI-0018F_5E_GAP_BY_CHAPTER.csv`",
            "- `docs/phase3/exports/LCAI-0018F_5E_CANDIDATE_INVENTORY.csv`",
            "- `docs/phase3/LCAI-0018F_GAP_ANALYSIS.md`",
            "",
            "## Prochaine étape (C1)",
            "",
            "1. Importer / valider les candidats 5e via Content Factory et quality gates existants.",
            "2. Revue pédagogique humaine distincte de l'approbation.",
            "3. Publication production — recalcul coverage — validation moteurs.",
            "",
        ]
    )
    (DOCS / "LCAI-0018F_PHASE0_BASELINE.md").write_text("\n".join(lines), encoding="utf-8")
...
def write_gap_analysis(gaps: list[dict[str, object]], candidate_inventory: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# LCAI-0018F — Analyse des écarts 5e (Gap Analysis C0)",
        "",
        f"- Chapitres curriculum 5e : **{len(gaps)}**",
    ...
    lines.append("")
    (DOCS / "LCAI-0018F_GAP_ANALYSIS.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    if matrix_rows:
        with (EXPORTS / "LCAI-0018F_5E_COVERAGE_MATRIX.csv").open("w", encoding="utf-8-sig", newline="") as handle:
            writer = csv.DictWriter(handle, fieldnames=list(matrix_rows[0].keys()))
            writer.writeheader()
            writer.writerows(matrix_rows)

    if gaps:
        with (EXPORTS / "LCAI-0018F_5E_GAP_BY_CHAPTER.csv").open("w", encoding="utf-8-sig", newline="") as handle:
            writer = csv.DictWriter(handle, fieldnames=list(gaps[0].keys()))
            writer.writeheader()
            writer.writerows(gaps)

    if candidate_inventory:
        with (EXPORTS / "LCAI-0018F_5E_CANDIDATE_INVENTORY.csv").open("w", encoding="utf-8-sig", newline="") as handle:
    ...
    print("LCAI-0018F Phase C0 audit complete")
