-- LCAI-0034 — coverage view with family diversity
...
SELECT
    ...
    COUNT(DISTINCT f.family_id) FILTER (WHERE f.family_id IS NOT NULL) AS unique_family_count,
    ...
    CASE
        ...
        WHEN COUNT(DISTINCT f.family_id) FILTER (WHERE f.family_id IS NOT NULL) < 8
             AND sk.brevet_importance = 'MEDIUM' THEN 'LOW_DIVERSITY'
        WHEN COUNT(DISTINCT f.family_id) FILTER (WHERE f.family_id IS NOT NULL) < 10
             AND sk.brevet_importance = 'HIGH' THEN 'LOW_DIVERSITY'
        WHEN COUNT(DISTINCT f.family_id) FILTER (WHERE f.family_id IS NOT NULL) < 12
...
LEFT JOIN v_content_items_effective c ON c.content_id = l.content_id
